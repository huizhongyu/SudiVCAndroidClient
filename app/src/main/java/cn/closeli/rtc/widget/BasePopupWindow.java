...
public abstract class BasePopupWindow extends PopupWindow {
    ...
    public void showAsLeft(View anchor) {
        ...
        showAtLocation(anchor, Gravity.NO_GRAVITY,
                location[0] - popupWidth, (location[1] + anchor.getHeight() / 2) - popupHeight / 2);

    }

    /**
     * 设置显示在v右边（以v的中心位置为开始位置）
     *
     * @param anchor
     */
    public void showAsRight(View anchor) {
        if (checkNonNull()){
            return;
        }
        if (isShowing()) {
        ...
        //在控件上方显示
        showAtLocation(anchor, Gravity.NO_GRAVITY,
                location[0] + popupWidth - anchor.getMeasuredWidth(), (location[1] + anchor.getHeight() / 2) - popupHeight / 2);
