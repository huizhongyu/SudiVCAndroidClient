...
import cn.closeli.rtc.utils.UnCeHandler;
...
public class App extends Application {
    ...
    public void onCreate() {
        ...
        try {
            Runtime.getRuntime().exec("setprop vhd.log.level 5");
        } catch (IOException e) {
            e.printStackTrace();
        }
        RoomManager.get().init(this);
        RoomClient.get().init(this);
        WebRTCManager.get().init(this);
        initCrash();

        CrashReport.initCrashReport(getApplicationContext(), "4bc863445c", BuildConfig.DEBUG);
